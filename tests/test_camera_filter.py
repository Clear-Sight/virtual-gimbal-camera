...
test_update_valid_input() - Tries different valid inputs to the update function.
test_update_not_valid_input() - Tries different unvalid inputs to the update function.
...
#@pytest.fixture
#def cameraFilter():
    #return Pipeline().camera_filter

#def test_not_valid_camera():
    #cf2 = Pipeline().camera_filter
    #with pytest.raises(ValueError, match=r".*No camera.*"):
        #cf2.start(3)
        #cf2.main()
...
def test_config():
    ...
    cam_input = config.CONFIG['cam_input']

def test_handle_frame():
    """Tests the function handle_frame().
    
    The function creates a matrix and puts it in 
    the handle_frame function. Then it tests for the
    right output. 
    """
    size = 50
    ...
    for pixelx in range(size):
        for pixely in range(size):
            img[pixelx][pixely] = [int((pixelx/size) * 255), int((pixely /size) * 255),  int(((pixelx + pixely) /(2*size)) * 255)]
    ...
    for test in tests:
        ...
        frame = cf.handle_frame(img, img.shape[0], img.shape[1],img.shape[0], img.shape[1])

        #frame.dump(f'tests/test_images/matrix{test[0]}-{test[1]}-{test[2]}.dat')
        loaded = np.load(f'tests/test_images/matrix{test[0]}-{test[1]}-{test[2]}.dat', allow_pickle=True)
